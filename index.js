	...
	self.load_apps = function(app_dir,root) {
		...
		app_directories(app_dir).forEach(function(dir) {
			...
			try {
              ...
              oWikiaHelper.getWords({limit:300}).then(function(sParagraph) {
...
//     self.express.use(verifier({ strictHeaderCheck: true }))
    
//     console.log("Another damn test" + self.express.headers.signature);
//     if (!self.express.headers.signature || self.express.headers.signature == undefined) {
//       console.error('missing Signature header:');

//       self.express.status(400).json({ status: 'failure7' });
//       self.express.session.destroy();
//       return false;
//     }
    ...
    //   res.status(400).json({ status: 'failure8'});
    //   req.session.destroy();
    //   return false;
    // }
